...
async def tour_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    intro = (
        "Список игроков на текущий тур! Выбери к себе в состав:\n"
        "- 3 нападающих\n"
        "- 2 защитников\n"
        "- 1 вратаря\n\n"
        "Назначь одного полевого игрока из состава капитаном (его очки умножим на х1.5)\n\n"
        f"Ваш бюджет: {budget}\n\n"
        f"Принимаем составы до: {deadline_str}"
    )
    ...
    try:
        ...
        if is_subscription_active(update.effective_user.id):
            premium_line = (
                "\n\n💎 Премиум: у тебя доступен персональный бонус — +1 игрок в пул (доступно: 1/1). Выбирай с умом!"
            )
            intro += premium_line
    except Exception:
        pass
